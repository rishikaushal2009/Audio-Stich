...
OUTPUT_FILE="$OUTPUT_DIR/test_$(date +%s).mp3"
...
CURL_OPTS=(-s -w "%{http_code}" -X POST "$API_URL" -H "Content-Type: application/json")
if [[ "$USE_API_KEY" == "true" ]]; then
    CURL_OPTS+=(-H "x-api-key: $API_KEY")
fi
CURL_OPTS+=(-d "{\"message\": \"$MESSAGE\", \"audios\": \"$BUCKET_NAME\", \"output\": \"$OUTPUT_FILE\"}")
...
if [[ "$HTTP_CODE" -eq 200 ]]; then
    ...
    if [[ "$SUCCESS" == "true" ]]; then
        ...
        if [[ -n "$AUDIO_DATA" && "$AUDIO_DATA" != "" && "$AUDIO_DATA" != "null" ]]; then
            ...
            LOCAL_FILE="$OUTPUT_DIR/stitched_$(date +%s).mp3"
...
if aws s3 ls "s3://$BUCKET_NAME/$OUTPUT_FILE" >/dev/null 2>&1; then
    ...
    LOCAL_FILE="$OUTPUT_DIR/s3_downloaded_$(date +%s).mp3"
